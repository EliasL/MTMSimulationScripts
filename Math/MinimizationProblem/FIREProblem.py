...
def find_minima(x_range, y_range, f_func, df_func):
    ...
    for x in x_range:
        for y in y_range:
            result = minimize(lambda x: f(x, f_func), [x, y], jac=lambda x: df(x, df_func), method='BFGS')
...
def is_new_minimum(point, minima, tol=1e-2):
    return not any(np.linalg.norm(np.array(point) - np.array(min_point)) < tol for min_point in minima)
...
def run_optimizations(x0s, f_func, df_func):
    ...
    for x0 in x0s:
        # FIRE algorithm
        result = optimize_fire2(x0, lambda x, params=None: f(x, f_func), lambda x, params=None: df(x, df_func), None, dt=0.01)
        ...
        # LBFGS algorithm
        LBFGS_path = []
        result = minimize(lambda x: f(x, f_func), x0, method='L-BFGS-B', jac=lambda x: df(x, df_func), callback=lambda xk: LBFGS_path.append(xk.copy()))
        LBFGS_paths.append(np.array([x0] + LBFGS_path))
        LBFGS_nit.append(result.nit)  # Extracting number of iterations directly from the result
...
def plot_results(X, Y, Z, minima, FIRE_paths, LBFGS_paths):
    ...
    for FIRE_path, LBFGS_path in zip(FIRE_paths, LBFGS_paths):
        y0 = f"{FIRE_path[0, 1]:.1f}" if FIRE_path[0, 1] % 1 else f"{FIRE_path[0, 1]:.0f}"
...
def main():
    ...
    FIRE_paths, LBFGS_paths, Fire_nit, LBFGS_nit, LBFGS_nfev = run_optimizations(initial_points, f_func, df_func)
