...
def plotPowerLaw(y_values_series, fig=None, ax=None, include_label=False, indicate_last_point=True, **kwargs):
    if ax is None:
        fig, ax = plt.subplots()
    ...
    ax.plot(bin_centers, fit.truncated_power_law.pdf(bin_centers), '--', color='r',
...
def makePlot(csv_file_paths, name, X=None, Y=None, x_name=None, y_name=None, labels=None,
             title=None, plot_average=False, xLims=(-np.inf, np.inf),indicateLastPoint=False,
    ...
    if plot_power_law:
        kwargs = {'fig': fig, 'ax': ax, 'label':'Fit', 'color':'black','include_label':legend}
    ...
    if isinstance(legend, list):
        custom_legend = [mlines.Line2D([], [], color='blue', marker='o', linestyle='-', label='Custom Label 1'),
                 mlines.Line2D([], [], color='green', marker='o', linestyle='--', label='Custom Label 2')]
...
def makePowerLawPlot(csv_file_paths, name, **kwargs):
    makePlot(csv_file_paths, name, X='Load', Y='Avg energy',
             x_name=f'Magnitude of energy drops', y_name='Frequency',
            title=f'Powerlaw',
             plot_raw=False, plot_power_law=True, plot_average=False, **kwargs)
